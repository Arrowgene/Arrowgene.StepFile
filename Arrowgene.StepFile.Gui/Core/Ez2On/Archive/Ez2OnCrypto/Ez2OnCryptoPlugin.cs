...
namespace Arrowgene.StepFile.Core.Ez2On.Archive.Ez2OnCrypto
{
    ...
    {
        public const int CRYPTO_TYPE_EZ = 1;

        private Logger _logger;

        public string Name => "Ez2OnCryptoPlugin";
        public string Version => "1.0";
        public int CryptoType => CRYPTO_TYPE_EZ;

        public bool CanDecrypt => true;

        public bool CanEncrypt => false;

        public Ez2OnCryptoPlugin()
        {
            _logger = LogProvider<Logger>.GetLogger(this);
        }

        public byte[] Decrypt(byte[] data, string fileExtension)
        {
            Ez2OnCryptoExtension? cryptoExt = GetCryptoExtension(fileExtension);
            if (cryptoExt == null)
            {
                _logger.Error($"Decrypt: Ez2OnCryptoExtension not found for {fileExtension}");
                return data;
            }
            Ez2OnCryptoKeyState keyState = Ez2OnCryptoKeyState.Get(cryptoExt);
            if (keyState == null)
            {
                _logger.Error($"Decrypt: Ez2OnCryptoKeyState not found for {cryptoExt}");
                return data;
            }
            return Decrypt(data, keyState);
        }

        public byte[] Encrypt(byte[] data, string fileExtension)
        {
            Ez2OnCryptoExtension? cryptoExt = GetCryptoExtension(fileExtension);
            if (cryptoExt == null)
            {
                _logger.Error($"Encrypt: Ez2OnCryptoExtension not found for {fileExtension}");
                return data;
            }
            Ez2OnCryptoKeyState keyState = Ez2OnCryptoKeyState.Get(cryptoExt);
            if (keyState == null)
            {
                _logger.Error($"Encrypt: Ez2OnCryptoKeyState not found for {cryptoExt}");
                return data;
            }
            return Encrypt(data, keyState);
        }

        private Ez2OnCryptoExtension? GetCryptoExtension(string extension)
        {
            if (!string.IsNullOrEmpty(extension))
            {
                switch (extension.ToLower())
                {
                    case ".png": return Ez2OnCryptoExtension.png;
                    case ".ogg": return Ez2OnCryptoExtension.ogg;
                    case ".jpg": return Ez2OnCryptoExtension.jpg;
                    case ".bin": return Ez2OnCryptoExtension.bin;
                    case ".pvi": return Ez2OnCryptoExtension.pvi;
                    case ".scr": return Ez2OnCryptoExtension.scr;
                    case ".bmp": return Ez2OnCryptoExtension.bmp;
                    case ".str": return Ez2OnCryptoExtension.str;
                    case ".dat": return Ez2OnCryptoExtension.dat;
                    case ".ptn": return Ez2OnCryptoExtension.ptn;
                }
            }
        ...
        private byte[] Decrypt(byte[] source, Ez2OnCryptoKeyState keyState)
        {
            UInt32 offset = 0;
            ...
            do
            {
                Ez2OnCryptoKeyState key1 = (Ez2OnCryptoKeyState)keyState.Clone();
                if (fsize >= 0x1000)
                {
                    fsize -= 0x1000;
                    currentptr += 0x1000;
                ...
                if (key1.CryptoExtension == Ez2OnCryptoExtension.ptn ||
                    key1.CryptoExtension == Ez2OnCryptoExtension.dat ||
                    key1.CryptoExtension == Ez2OnCryptoExtension.str)
                {
        ...
        private byte[] Encrypt(byte[] source, Ez2OnCryptoKeyState keyState)
        {
            UInt32 offset = 0;
            UInt32 fsize = (UInt32)source.Length;
            UInt32 currentptr = 0;
        ...
        private void Permutation(Ez2OnCryptoKeyState keyState)
        {
            switch (keyState.CryptoExtension)
            {
                case Ez2OnCryptoExtension.png:
                case Ez2OnCryptoExtension.ogg:
                case Ez2OnCryptoExtension.jpg:
                case Ez2OnCryptoExtension.bin:
                case Ez2OnCryptoExtension.pvi:
                case Ez2OnCryptoExtension.scr:
                case Ez2OnCryptoExtension.bmp:
                    fun_4d7030(keyState);
                    break;
                case Ez2OnCryptoExtension.str:
                case Ez2OnCryptoExtension.dat:
                case Ez2OnCryptoExtension.ptn:
                    fun_4D8160(keyState);
                    break;
            }
        }

        private UInt32 Out(Ez2OnCryptoKeyState keyState, int index)
        {
            switch (keyState.CryptoExtension)
            {
                case Ez2OnCryptoExtension.png:
        ...
        private UInt32 OutE(Ez2OnCryptoKeyState keyState, int index)
        {
            switch (keyState.CryptoExtension)
            {
                case Ez2OnCryptoExtension.png:
        ...
        private void Assign(Ez2OnCryptoKeyState keyState, int index)
        {
            switch (keyState.CryptoExtension)
            {
                case Ez2OnCryptoExtension.png:
        ...
        private void AssignE(Ez2OnCryptoKeyState keyState, int index, UInt32 _out)
        {
            switch (keyState.CryptoExtension)
            {
                case Ez2OnCryptoExtension.png:
        ...
        private void fun_4D8160(Ez2OnCryptoKeyState keyState)
        {
        ...
        private void fun_4d7030(Ez2OnCryptoKeyState keyState)
        {
